...
| `HS_FB_ROTA_TOKEN` | Optional, 24+ characters. Bearer token for the groups runner's two doors, `GET /api/fb/rota` and `POST /api/fb/rota/result` (`api/rota.go`); unset = the doors do not exist. The same value goes into `scripts/fb-groups/.env` on the machine that runs the browser. `docs/facebook.md`, *The runner*. |
