...
{{range $d.Sources}}{{$s := .}}<tr {{if not .Enabled}}style="opacity:.55"{{end}}><td><b>{{.Label}}</b><br><a class="small" href="{{.URL}}" rel="noopener noreferrer" target="_blank">{{short .URL 70}}</a></td><td>{{.Kind}}</td><td class="small">{{town .Town}}<br>{{cat .Category}}{{if .Listing}}<br><span class="mono">{{.Listing}}</span>{{end}}{{if .Match}}<br><span class="mute" title="Only events matching this pattern are queued">filter: <span class="mono">{{short .Match 40}}</span></span>{{end}}</td><td>{{ago .Checked}}{{if .Changed}}<br><span class="small mute">changed {{ago .Changed}}</span>{{end}}</td><td class="small">{{if .Status}}<span class="pill {{if hasPrefix .Status "error"}}no{{else if eq .Status "changed"}}warn{{else}}ok{{end}}">{{short .Status 60}}</span>{{else}}<span class="mute">not yet checked</span>{{end}}</td><td class="num">{{.Events}}</td>
...
<label>Only queue events matching (optional, ics feeds only)</label><input type="text" name="match" placeholder="somerset|strand|gordon|helderberg|sir lowry">
