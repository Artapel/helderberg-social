...
<td>{{if ne .Status "active"}}<span class="pill no">disabled</span>{{else if .VerifiedAt}}<span class="pill ok">confirmed</span>{{else}}<span class="pill warn">unconfirmed</span>{{end}}{{if .GoogleSub}} <span class="pill">Google</span>{{end}}</td><td class="num">{{.Events}}</td><td>{{ago .CreatedAt}}</td><td>{{ago .LastLoginAt}}</td>
...
<div class="panel"><p><b>{{$m.Name}}</b> &middot; {{$m.Email}} &middot; {{if ne $m.Status "active"}}<span class="pill no">disabled</span>{{else if $m.VerifiedAt}}<span class="pill ok">confirmed</span> {{ts $m.VerifiedAt}}{{else}}<span class="pill warn">email not confirmed</span>{{end}}{{if $m.GoogleSub}} &middot; <span class="pill">Google</span>{{if not $m.HasPassword}} <span class="mute small">(no password)</span>{{end}}{{end}}</p>
...
<div class="panel"><h3>Sign in with Google {{if $d.GoogleOn}}<span class="pill ok">on</span>{{else}}<span class="pill">off</span>{{end}}</h3>
{{if $d.GoogleOn}}<table><tr><td>Client id</td><td class="mono">{{$d.GoogleClientID}}</td></tr><tr><td>Redirect URI</td><td class="mono">{{$d.GoogleRedirect}}</td></tr><tr><td>Members linked</td><td>{{$d.MembersGoogle}}</td></tr></table><p class="small mute">OpenID Connect with PKCE; Google confirms the address, so these members skip the confirmation mail. The redirect URI above must be listed on the OAuth client in Google Cloud exactly. Runbook: <span class="mono">docs/accounts.md</span>.</p>
{{else}}<p class="small mute">Not configured. Set <span class="mono">HS_GOOGLE_CLIENT_ID</span> and <span class="mono">HS_GOOGLE_CLIENT_SECRET</span> (runbook in <span class="mono">docs/accounts.md</span>). The sign-in and register pages hide the Google button while this is off.</p>{{end}}</div>
